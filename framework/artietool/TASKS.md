...
Each component owns the definitions of the tasks that build, test, flash and deploy it,
under its own directory:

```
<component>/.artie/tasks/build-tasks/**/*.yaml
<component>/.artie/tasks/test-tasks/**/*.yaml
<component>/.artie/tasks/deploy-tasks/**/*.yaml
<component>/.artie/tasks/flash-tasks/**/*.yaml
```

Artie Tool discovers them across every component in the workspace, so it does not need to
know what any component contains - only where its checkout is. Task names are the
subcommands a user types, so they have to be unique across the whole workspace; two
components claiming the same name is an error. A task can also be named unambiguously as
`<component>:<task>`, for example `ardk:artie-base-image`.
...
- *chart*: Path to the chart, relative to the root of the component that owns this task. E.g., `deploy/artie-base`, OR `${CHART_PATH}`
...
- `REPO_ROOT`: The root of the component that declares this task. Because a task lives in
               the component it builds, this lets a task refer to its own files with a
               plain relative path, and stay correct once that component moves to its own
               repository.
- `REPO:<name>`: The root of a *different* component, as in `${REPO:ardk}/firmware/libraries/leds`.
                 Reaching across a component boundary has to be spelled out this way, so
                 that every such dependency is visible in the task file rather than implied
                 by everything happening to sit under one directory. Run
                 `artie-tool workspace status` to see the component names and where each
                 one currently resolves to.
- `GIT_TAG`: The git hash (short form) of the component that declares this task.
