...
class HWConfig:
    ...
    def _from_config_v1(raw):
        ...
        if "single-board-computers" not in raw:
            raise KeyError(f"Cannot find 'single-board-computers' in Artie type file: {fpath}")

        if "microcontrollers" not in raw:
            raise KeyError(f"Cannot find 'microcontrollers' in Artie type file: {fpath}")

        if "actuators" not in raw:
            raise KeyError(f"Cannot find 'actuators' in Artie type file: {fpath}")
