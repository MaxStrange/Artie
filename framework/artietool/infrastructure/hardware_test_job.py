...
class CollectedHardwareTestSteps:
    ...
    def _convert_test_into_script(self, args, artie_id: str, test_def: test_job.HWTest) -> str:
        ...
        for test_cmd in test_def.cmds_to_run_in_cli:
          ...
          contents += f"{cmd} | python /interpret_test_output.py || EXIT_STATUS=$?\n"
    ...
    def _determine_test_script_contents(self, args, artie_id: str, leading_spaces: int) -> str:
        ...
        contents += "set -uo pipefail\n"
        contents += "EXIT_STATUS=0\n"
        ...
        # Postamble...?
        contents += "exit $EXIT_STATUS"

        indented_contents = ""
        for line in contents.splitlines():
            indented_contents += " " * leading_spaces + line.rstrip() + "\n"

        common.info(f"Test Script Autogenerated:\n{indented_contents}")
...
"""
    ...
    def combine(self, other) -> None:
        """
        Combines this CollectedHardwareTestSteps object with the other one,
...
class HardwareTestJob(test_job.TestJob):
    ...
    def combine(self, other) -> None:
        """
        Combine this task with the given HardwareTestJob.
        """
        self.steps[0].combine(other._single_step)
