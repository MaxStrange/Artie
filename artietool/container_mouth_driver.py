"""
Stuff pertaining to the mouth driver container.
"""
from . import common
...
MOUTH_DRIVER_BUILD_TASK_NAME = "container-mouth"

def _build_mouth_driver(args) -> common.BuildResult:
    """
    Build the mouth driver container image.
    """
    common.set_up_logging(args)
    logging.info("Building mouth driver container image...")
    ...
    docker_image_name = f"{repo_prefix}artie-mouth-driver:{git_tag}"
    logging.info(f"Building Docker image and tagging it as {docker_image_name}")

    # Retrieve dependencies
    fw_image = args._artifacts[fw_mouth.MOUTH_BUILD_TASK_NAME][common.ArtifactTypes.DOCKER_IMAGE]
    logging.info(f"Using {fw_image} as the base Docker image for driver build.")

    # Build the Docker image
    dockerargs = f"--build-arg DRIVER_TYPE=mouth --build-arg FW_IMG={fw_image} --build-arg RPC_PORT=4243"
    ...
    return common.BuildResult(
        name=MOUTH_DRIVER_BUILD_TASK_NAME,
        success=True,
        artifacts={
...
BUILD_CHOICES = set([
    common.BuildTask(
        name=MOUTH_DRIVER_BUILD_TASK_NAME,
        build_function=_build_mouth_driver,
        artifacts=set([
            common.ArtifactTypes.DOCKER_IMAGE,
        ]),
        dependencies={
            "fw-mouth": [common.ArtifactTypes.DOCKER_IMAGE],
        },
        clean_function=_clean_mouth_driver
    )
])
