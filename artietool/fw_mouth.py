"""
Build module for the mouth MCU FW.
"""
from . import common
import logging
import os
...
MOUTH_BUILD_TASK_NAME = "fw-mouth"

def _build_mouth(args) -> common.BuildResult:
    """
    Build the mouth FW.
    """
    common.set_up_logging(args)
    logging.info("Building mouth firmware...")

    # Set the location to the fw-mouth directory
    build_directory = os.path.abspath(os.path.join(common.repo_root(), "firmware", "mouth", "build"))
    logging.info(f"Building from {build_directory}")

    # Docker image name
    docker_image_name = common.construct_docker_image_name(args, "artie-mouth")
    ...
    artifacts = {
        ...
        common.ArtifactTypes.FILES: [f"{args.artifact_folder}/mouth.{suffix}" for suffix in suffixes],
    }

    return common.BuildResult(
        name=MOUTH_BUILD_TASK_NAME,
        success=True,
        artifacts=artifacts,
    )

def _clean_mouth(args):
    """
