...
def run(config: myconf.Configuration, savedir: str):
   ...
   audiodpath = config.getpath('ARTIE', 'audio-source')
   wavs = datasets.read_audio_files(audiodpath)
   spectrograms = datasets.run_config_pipeline(wavs, config)

   # Create the autoencoder from the config file specifications
   autoencoder = networks.make_network(config, 'AUTOENCODER')

   # Train the autoencoder (unless already trained)
   autoencoder.load(config)

   # Create the DDSP synthesizer network from the config file specifications
   synthesizer = networks.make_network(config, 'SYNTHESIZER')

   # Pretrain the synthesizer network to make babbling noises (maybe)
   synthesizer.load(config)
   ...
   super_network.train(config, spectrograms)
